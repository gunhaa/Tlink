const canvas = document.querySelector("#whiteboard") as HTMLCanvasElement;
    ...
    if (drawing) {
        ...
        if (arrXY.length < 30) {
            ...
            for (let i = 0; i < arrXY.length; i += 30) {
                ...
                await window.opener.postMessage(getDrawDataAsJson(), '*');
            }
            drawData.length = 0;
            arrXY.length = 0;
        }
...
const drawFromData = (drawdata: any[]) => {
